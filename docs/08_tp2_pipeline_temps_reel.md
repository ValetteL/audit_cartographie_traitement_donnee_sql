...
`spark/jobs/clean_and_load.py` tourne en boucle (`local[*]`, pas de cluster nécessaire), relit `aggregated/dpe_enrichi.ndjson` toutes les 60s et applique :
...
Démarre l'ensemble (TP1 + TP2) : base PostgreSQL et son import TP1, data lake, Kafka, le pipeline complet (producteur, agrégateur, Spark), Prometheus, Grafana, cAdvisor et Metabase — sans étape manuelle ni accès réseau obligatoire (le backfill utilise les CSV déjà versionnés ; seule la phase "live" de `api-producer` appelle l'API ADEME, avec repli silencieux si elle est inaccessible). Pour repartir d'un état vide : `docker compose down -v`.
